...
- **spark.sql.files.maxPartitionBytes**：指定 Spark SQL 读取文件时每个分区的最大大小，可以影响 partition 数，默认 128MB，仅对基于文件的数据源生效，如 parquet、json、orc。
...
- **spark.shuffle.file.buffer**： Map 输出端写入缓冲区大小，默认 32KB，通过增大 buffer ，减少落盘频率
- **spark.io.compression.lz4.block Size**： 压缩时的 block 大小，默认 32 KB，增大该值可以减小 Shuffle file 大小，推荐 512KB
- **spark.reducer.maxSizeInFlight**： Reduce 端的读取缓冲区大小，默认 48MB，通过增大 buffer 增加每次拉拉取的数据，减少网络请求
- **spark.shuffle.sort.bypassMergeThreshold**：默认 `200`，当 Reducer 并行度低于该阈值时，Shuffle 在 Map 端均不会引入排序，但每个 partition 都需要维护一个 buffer

关于 Bypass 与 Unsafe 的：

- **spark.file.transferTo**： 开启后会以 NIO 传输文件，默认 true
- **spark.shuffle.unsafe.file.output.buffer**： 在 Shuffle 期间合并文件可用的 buffer，默认 32KB，对于高负载推荐 1MB

同时还有一些稳定性参数设置：

- **spark.shuffle.compress**：开启 shuffle 压缩，默认开启
- **spark.io.compression.codec**：指定 shuffle 中间文件压缩所使用压缩类型，默认 lz4
...
- **spark.sql.adaptive.coalescePartitions.minPartitionNum**：指定 RDD 最小分区数，避免无法充分利用 CPU，不指定则取默认并行度。
